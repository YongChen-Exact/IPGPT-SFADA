...
from utils import ramps, util
from val_2D import test_single_volume_fast

os.environ["CUDA_VISIBLE_DEVICES"] = "0"
parser = argparse.ArgumentParser()
parser.add_argument("--root_path", type=str, default="/home/data/CY/Datasets/MMS/Heart_B", help="Name of Experiment")  # 数据集的位置
parser.add_argument("--model", type=str, default="unet", help="model_name")
parser.add_argument("--model_path", type=str,
                    default="/home/data/CY/codes/IPGPT-SFADA/Code_OA/Model/Heart_A_unet/unet_best_model.pth")
parser.add_argument("--active_path", type=str,
                    default="/home/data/CY/codes/IPGPT-SFADA/Code_OA/Results/selection_list/Heart_B.txt",
                    help="Source_model_name")
parser.add_argument("--source", type=str, default="Heart_A", help="Source_model_name")
parser.add_argument("--target", type=str, default="Heart_B", help="experiment_data name")
...
def train(args, snapshot_path):
    ...
    labeled_dataset = active_DataSets(base_dir=args.root_path, split="train", transform=RandomGenerator(args.patch_size), active_path=args.active_path)
    unlabeled_dataset = unlabeled_DataSets(base_dir=args.root_path, transform=RandomGenerator(args.patch_size), active_path=args.active_path)
    ...
    stu_model.load_state_dict(torch.load(args.model_path)["state_dict"])
    tch_model.load_state_dict(torch.load(args.model_path)["state_dict"])
    ...
    model.train()
    dice_loss = DiceLoss(num_classes).to(torch.device('cuda'))
    ...
    for epoch_num in iterator:
        ...
        for i_batch, (labeled_data, unlabeled_data) in enumerate(zip(labeled_loader, unlabeled_loader)):
            ...
            un_image_batch, un_label_batch, un_name_batch = (unlabeled_data["image"],unlabeled_data["label"],unlabeled_data["name"])
            un_image_batch, un_label_batch = (un_image_batch.cuda(), un_label_batch.cuda())
            _, outputs = model(image_batch)
            outputs_soft = torch.softmax(outputs, dim=1)
            supervised_loss1 = dice_loss(outputs_soft, label_batch)
            outputs_emas = []
            model.ema_model.train()
            for _ in range(mc_iteration):
                with torch.no_grad():
                    _, output = model.ema_model(un_image_batch)
                    outputs_emas.append(output)
            all_predictions = torch.stack(outputs_emas)
            std_dev = torch.std(all_predictions, dim=0)
            uncer_th = torch.mean(std_dev)
            outputs_ema = torch.stack(outputs_emas).mean(0)
            probs_w, pseudo_labels = torch.nn.functional.softmax(outputs_ema, dim=1).max(1)
            ...
            supervised_loss2 = dice_loss(un_outputs_soft, pseudo_labels)
            difficulty_score = uncer_th
            ...
            if iter_num > 0 and iter_num % 100 == 0:
                ...
                with torch.no_grad():
                    for i_batch, sampled_batch in enumerate(valloader):
                        metric_i = test_single_volume_fast(sampled_batch["image"], sampled_batch["label"], model,classes=num_classes, patch_size=args.patch_size)
                ...
                logging.info("iteration %d, mean_dice: %f, LV: %f, MYO: %f, RV: %f" % (iter_num, performance, metric_list[0], metric_list[1], metric_list[2]))
            model.train()
            if iter_num % 3000 == 0:
...
if __name__ == "__main__":
    ...
    snapshot_path = "/home/data/CY/codes/IPGPT-SFADA/Model/{}_to_{}".format(args.source, args.target)
