...
class BaseDataSets(Dataset):
    def __init__(self, base_dir=None, split="train", transform=None, train_mixed=None):
        self._base_dir = base_dir
        self.sample_list = []
        self.split = split
        self.transform = transform
        if self.split == "train":
            self.mixed_path = base_dir + train_mixed
            self.sample_list = sorted(os.listdir(self.mixed_path))
    ...
    def __getitem__(self, idx):
        ...
        if self.split == "train":
            h5f = h5py.File(self.mixed_path + "{}".format(case), "r")
        else:
...
class DataSets_like(Dataset):
    def __init__(
        ...
        self.transform = transform
        if self.split == "train":
            self.sample_list = os.listdir(self._base_dir + "/source_like/")
        elif self.split == "val":
            self.sample_list = os.listdir(self._base_dir + "/val_set/")
        print("total {} samples".format(len(self.sample_list)))
    ...
    def __getitem__(self, idx):
        case = self.sample_list[idx]
        if self.split == "train":
            h5f = h5py.File(self._base_dir + "/source_like/{}".format(case), "r")
        else:
            h5f = h5py.File(self._base_dir + "/val_set/{}".format(case), "r")
        image = h5f["image"][:]
        label = h5f["label"][:]
