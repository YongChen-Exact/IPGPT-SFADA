...
def Savefeat():
    class_num = 2
    db_source_like = DataSets_like(base_dir="/home/data/CY/Datasets/Prostate_HCRUDB", split="train", transform=RandomGenerator([256, 256]))
    source_like_loader = DataLoader(db_source_like, batch_size=1, shuffle=False, num_workers=8, pin_memory=True)
    db_target = BaseDataSets(base_dir="/home/data/CY/Datasets/Prostate_HCRUDB", split="train", transform=RandomGenerator([256, 256]))
    ...
    model.load_state_dict(torch.load('/home/data/CY/codes/IPGPT-SFADA/Model/Prostate_RUNMC_BMC_unet/unet_best_model.pth')["state_dict"])
    ...
    selected_list = select_IPL(selected_num, class_num, s_reprs, s_logits, t_reprs, t_logits, t_inds, uncertainties, int(math.sqrt(len(target_loader))))
    selected_cases = []
    for idx in selected_list:
        selected_cases.append(str(t_idx_names[idx])[2:-2])
    file = os.path.join('/home/data/CY/codes/IPGPT-SFADA/Results/selection_list', f'Prostate_HCRUDB.txt')
